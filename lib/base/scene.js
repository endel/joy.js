...
(function(J) {
  var Scene = J.DisplayObjectContainer.extend({
    ...
    /**
     * Create scene loader, which will display while this scene is loading.
     *
     * @example
     *    scene.loading(function(loadScreen, progress) {
     *      loadScreen.addChild(new Joy.Text({
     *        text: "Loading..."
     *      }).bind(Joy.Events.UPDATE, function() {
     *        this.text = "Loading... " + loading.percentage + "%";
     *      }))
     *    };
     *
     * @param {Function} fn function to setup loader scene
     */
    loading: function (fn) {
      var that = this;

      J.currentEngine.createScene(function(scene) {
        fn.apply(this, [scene, that.loader]);
      });
    },
