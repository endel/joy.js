...
(function(J){
    ...
    this.parsers = {
      ...
      DIV: function (el, dataset) {
        ...
        for (var i = 0, l = children.length; i < l; i ++) {
          displayObject.addChild( this.parsers[ children[i].tagName ].apply(this, [children[i], this.evaluateDataset(children[i])]) );
        }
  ...
  Markup.prototype.createScene = function(section) {
    ...
    for (i=0, length=elements.length;i<length; ++i) {
      if (this.parsers[elements[i].tagName]) {
        var el = this.parsers[elements[i].tagName].apply(this, [elements[i], this.evaluateDataset(elements[i])]);
        console.log(el);
        children.push(el);
      }
    }
  ...
  Markup.prototype.evaluateDataset = function(el) {
